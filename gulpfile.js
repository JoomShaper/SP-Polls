...
function cleanZip() {
	return src('./com_sppolls_v3.2.0.zip', { read: false, allowEmpty: true }).pipe(clean());
}

function copySite() {
	return src([
		'./components/com_sppolls/**/*.*',
...
function copy_lang_site() {
	return src('./language/en-GB/en-GB.com_sppolls.ini').pipe(dest('build/language/site/en-GB'));
}

function copy_lang_admin() {
	return src([
		'./administrator/language/en-GB/en-GB.com_sppolls.ini',
		'./administrator/language/en-GB/en-GB.com_sppolls.sys.ini',
	]).pipe(dest('build/language/admin/en-GB'));
}

function copy_modules() {
	return src('./modules/mod_sp_poll/**/*.*').pipe(dest('build/modules/mod_sp_poll'));
}

function copy_modules_lang() {
	return src('./language/en-GB/en-GB.mod_sp_poll.ini').pipe(
		dest('build/modules/mod_sp_poll/language')
	);
}
...
function makeZip() {
	return src('./build/**/*.*').pipe(zip('com_sppolls_v3.2.0.zip')).pipe(dest('./'));
}
